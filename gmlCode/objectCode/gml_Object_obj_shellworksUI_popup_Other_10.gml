with(obj_shellworksUI_popup){
    if(id != other.id){
        if(my_popup_id == other.my_popup_id){
...
if(!just_drew){
    ...
    for(var i = 0; i < array_length(buttons); i++){
        ...
        switch butType {
            ...
            case 3:
                var functionToRun = but[2]
                var functionArguments = but[3]
                if(gml_Script_shellworks_imgui_newbutton_doFunc(butLabel, functionToRun, functionArguments)){
                    gml_Script_shellworks_imgui_close_popup()
                }
                break;
        }
